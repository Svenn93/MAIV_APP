...
- (void)groupButtonPressed:(id)sender
{
    ...
    [self.delegate gebouwSelectedMetId:(int)pressedButton.tag];
    
    /*for (IndicatorView *indicV in self.arrIndicators) {
        
        if(indicV.gebouwid == (int)pressedButton.tag)
        {
            indicV.aantal++;
            [indicV setNeedsDisplay];
            
            if(indicV.aantal == indicV.maxaantal)
            {
